...
export interface GameSettingsSectionProps {
  ...
  localizeField?: (field: GameSettingField) => GameSettingField;
}
...
export function GameSettingsSection({
  ...
  const rawFields = useRef<GameSettingField[]>([]);
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      rawFields.current = data.settings;
      const localized = data.settings.map(localizeField);
      setFields(localized);
      const initial: Record<string, string | number | boolean> = {};
      localized.forEach((f) => { initial[f.key] = f.value; });
      setEdits(initial);
    } catch (err: any) {
  ...
  }, [serverId, canRead, load, localizeField]);
  ...
  useEffect(() => {
    if (rawFields.current.length > 0) {
      setFields(rawFields.current.map(localizeField));
    }
  }, [localizeField]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (Object.keys(changed).length === 0) {
        setSuccess(t('gameSettings.noChanges', 'No changes to save.'));
        setSaving(false);
        return;
      }
      ...
      setSuccess(
        editableOnlyWhenStopped
          ? t('gameSettings.savedNextStart', 'Settings saved. They will apply on the next start.')
          : t('gameSettings.savedRestart', 'Settings saved. Restart the server to apply the changes.')
      );
      await loadSettings();
    } catch (err: any) {
  ...
  return (
    <SectionCard title={t('gameSettings.title', 'Server Settings')} borderColor={borderColor} contentBg={contentBg} textPrimary={textPrimary}>
      {editableOnlyWhenStopped
        ? isRunning && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30 text-sm text-amber-300">
              ...
              <span>{t('gameSettings.stopToEdit', 'The server must be stopped to edit these settings. Stop it, make your changes, then start it again.')}</span>
            </div>
          )
      ...
      {loading && (
        <div className="flex items-center gap-2 text-sm text-gray-400">
          ...
          {t('gameSettings.loading', 'Loading settings...')}
        </div>
      )}
      ...
      {!loading && fields.length === 0 && !error && (
        <p className={`text-sm ${textSecondary}`}>
          {t('gameSettings.none', 'No settings available. The server may not have started yet.')}
        </p>
      )}
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div className="relative w-full sm:max-w-md">
                ...
                <AppInput
                  ...
                  placeholder={t('gameSettings.search', 'Search settings by name or key…')}
                  className="w-full pl-9 pr-3 py-2 bg-gp-surface-elevated border border-gray-600 rounded text-white"
                />
              </div>
              <span className={`text-xs ${textSecondary}`}>
                {t('gameSettings.showing', 'Showing {shown} of {total}', { shown: filteredFields.length, total: fields.length })}
              </span>
            </div>
          )}
          {filteredFields.length === 0 && (
            <p className={`text-sm ${textSecondary}`}>{t('gameSettings.noMatch', 'No settings match your search.')}</p>
          )}
          ...
          {canWrite && (
            <div className="flex justify-end pt-2">
              ...
              >
                ...
                {saving ? t('gameSettings.saving', 'Saving…') : t('gameSettings.save', 'Save')}
              </AppButton>
            </div>
          )}
        </>
